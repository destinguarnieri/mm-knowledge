...
- **Current VWAP state:** no immediate-execution mapping is promoted. Both the whole-signal continuous control and corrected threshold-band identity failed the short forward window. The new continuous excursion-geometry matrix is complete across 30 assets, `5m`/`15m`/`1h`, and three matched chronological blocks; interpretation is the next step before any position-curve experiment.
...
EMA 10/200 is restored as a parent research program. Band-to-band traversal is one supported branch alongside the flip-only controls, thresholds, signal-stat market state, continuous/nonlinear positioning, and asset/timeframe selection. MON-210's point-in-time `>$250,000` Hyperliquid-volume universe remains frozen at 100 assets, but the 2026-08-07 fixed-floor execution is wholly invalid: many assets returned zero/no metrics and Destin identified the `emac_cross` strategy file as broken. MON-210 is blocked pending Destin's repair and verification; none of those batch metrics or interpretations may be used. Continuous `emac` V1 still lacks a clean standalone baseline, V4 economics are sparse, and V5 has no backtest. MON-165 remains blocked on a valid MON-211 baseline.
...
- Linear is rebuilt for parallel execution. EMA 10/200 control MON-210 is Blocked on Destin's `emac_cross` repair after invalid zero/no-metric output; MON-211 remains Ready, and MON-165 is dependency-blocked on MON-211 before matched V4/V5 work. EMA branches MON-157/167, EWMAC MON-166, bounded accumulation/distribution schedules MON-168, Catalog queue MON-169, multi-signal forecasting MON-170, and EMA/PX agent-vision MON-171 remain Ready. Human-labeled EMA/PX MON-172 remains blocked on Destin semantics.
...
- Continuous VWAP excursion geometry: 270/270 matched cells completed for 30 Binance-supported liquid assets across `5m`/`15m`/`1h` and development/replication/confirmation blocks. Canonical KB artifact `796b64a5`; causal pre-roll and left-censor exclusion are enforced. Fifteen focused tests plus Ruff/format/mypy pass.
...
For VWAP, interpret canonical continuous-excursion artifact `796b64a5` first, emphasizing cross-role/timeframe stability, side asymmetry, fade return, recross, and additional adverse excursion by actual continuous depth. Only after that read should one depth/capacity hypothesis be preregistered for MON-168's offline allocator comparison. Keep readiness features, curve tuning, and live wiring out of this evidence step; `px_signal_bands` remains a binary correctness control rather than the destination design.

For EMA 10/200, stop MON-210 until Destin repairs and verifies the broken `emac_cross` strategy. Discard the attempted fixed-floor batches as evidence; after repair, rerun from the existing frozen 100-asset population and fixed assumptions. Do not start downstream comparisons from the invalid runs. MON-211 remains the independent continuous-V1 control, and MON-165 must wait for valid matched cells before V4/V5 attribution.
