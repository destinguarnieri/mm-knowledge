...
- Visual-selection risk: all patterns were identified on one favorable window; nothing is promoted until event rules are causal and tested on fixtures not chosen from these screenshots.
...
1. **Mean lines (white) as trade filter.** Signal beyond the mean = something interesting is happening; inside the mean = no strong trend or breakout. Candidate binary permission state.
