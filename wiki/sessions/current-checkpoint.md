...
For VWAP, Destin must manually add `px_signal_bands` to the UI strategy catalog. Then run one bounded full-retention smoke (at most 5,000 candles in an individual job) and inspect entry/exit correctness plus all eight market-state panes before launching the long summary economics. Use the event-study's roughly coin-flip inward 2σ→1σ prior as the baseline expectation; keep escalation, slope/readiness rules, PnL-aware resizing, and wide-stop policy separate until the baseline view is understood.
