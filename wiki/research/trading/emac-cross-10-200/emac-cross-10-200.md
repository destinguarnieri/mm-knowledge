...
Testing whether the near-parameter-free EMA 10/200 family carries tradable edge, and how to monetize it. **Directional and traversal evidence remains promising. The frozen 100-name liquid-universe 1D slice executed for repaired flip-only `emac_cross`, direct continuous `emac` V1, fixed-`±0.01` transition-only V4, and fixed-`±0.01` continuous V5, but its fixed-floor windows are now classified as regime-confounded diagnostics rather than admissible lifespan/population evidence. The four mappings remain paired within each executed asset-window; breadth, winner identity, correlations, medians, and generalization claims are quarantined pending planner redesign and rerun.**
...
- **The 1D version ladder is matched but window-confounded.** Every strategy completed the same 96 planned Binance USD-M asset-window cells, so same-asset/same-window execution and mapping differences remain diagnostic evidence. The planner assigned each asset to the largest fixed floor it cleared and set `start_ms = common_end_ms - floor × interval_ms`, silently discarding up to the gap below the next floor. At 1D the 2,500-bar window begins 2019-10-03 while the 2,000-bar window begins 2021-02-14, omitting a market-defining regime from assets that had more than 2,000 but fewer than 2,500 contiguous bars. Do not use the prior breadth/median read as lifespan or population evidence.
- **Overlap/correlation and threshold sensitivity are paused behind planner design.** The similar positive counts may reflect shared trendability, mapping-specific winners, or arbitrary regime truncation. Calculating winner overlap/correlation on these windows would precisely analyze the avoidable planner artifact. The preregistered `±0.05`/`±0.10` runs are also paused.

**Best next step:** pause EMAC expansion and resolve [MON-214](https://linear.app/money-machine/issue/MON-214/redesign-binance-batch-planning-to-preserve-auditable-crypto-history), which owns the planner's research estimand and history-retention contract. The current implementation discovers trailing contiguous availability backward from a fixed common end, then replaces each asset's available start with a synthetic floor start. Decide how much available calendar history a shared batch may discard, require that loss to be visible per asset, then re-plan before any rerun. Do not calculate overlap/correlation or run `±0.05`/`±0.10` on the current fixed-floor windows.
...
| (D) Is the EMAC version ladder comparison-ready? | No; 1D execution is paired but window-confounded | High | Four variants completed the same 96 cells, but fixed floors discarded materially different market regimes | planner retention contract, audited re-plan, then rerun affected cells |
...
|---|---|---|---|
| Basic flip-only `emac_cross` | Causal repair verified on persisted ETH fixture; fixed-floor 1D control executed 96/96 planned cells | Fixed-floor windows are not admissible lifespan/population evidence; planner redesign and rerun required | [MON-210](https://linear.app/money-machine/issue/MON-210/complete-the-multi-asset-1d-emac-cross-control) — window-confounded diagnostic runs |
| Unthresholded continuous `emac` V1 | Direct-position V1 executed on the same 96 fixed-floor cells | 1D rerun after planner decision; `5m`/`1h`/`4h` remain | [MON-211](https://linear.app/money-machine/issue/MON-211/establish-the-unthresholded-continuous-emac-v1-baseline) — window-confounded 1D diagnostics |
| Transition-only thresholded `emac_v4` | Fixed `±0.01` executed on the same 96 fixed-floor cells | Planner redesign/rerun before overlap, threshold sensitivity, or remaining timeframes | [MON-165](https://linear.app/money-machine/issue/MON-165/compare-thresholded-continuous-emac-v5-with-continuous-v1-and-v4) — window-confounded 1D diagnostics |
| Thresholded continuous `emac_v5` | Fixed `±0.01` executed on the same 96 fixed-floor cells | Same boundary as V4; full MON-211 dependency remains | [MON-165](https://linear.app/money-machine/issue/MON-165/compare-thresholded-continuous-emac-v5-with-continuous-v1-and-v4) — window-confounded 1D diagnostics |

## Open Threads / Next Experiments

Ranked by expected information value:

1. **Redesign the batch planner's research contract in [MON-214](https://linear.app/money-machine/issue/MON-214/redesign-binance-batch-planning-to-preserve-auditable-crypto-history).** The current start is synthetic: availability counts contiguous bars backward from `common_end_ms`, fixed floors assign the largest floor cleared, and each plan computes `start_ms = common_end_ms - n_bars × interval_ms`. Define a maximum permitted loss of available calendar history, expose available-versus-executed start/loss per asset, and accept more/smaller batches when required by crypto's episodic regimes.
2. **Audit a plan before rerunning.** Produce plan-only output under the accepted retention contract. Verify the actual common end, each asset's trailing contiguous availability, executed start, discarded days/bars, and explicit exclusions. Stop if a batch boundary removes a materially distinct regime.
3. **Rerun the affected 1D ladder before attribution.** Only after clean windows exist should winner overlap/correlation and the `±0.05`/`±0.10` threshold prediction be tested.
4. **Complete the version ladder beyond 1D only under the accepted planner.** MON-211 still requires direct V1 at `5m`/`1h`/`4h`; MON-165 remains dependency-blocked for its full verdict.
4. **Capture the 80%.** Independently spec a bounded mean-cycle variant (S1 entry / S4 mean-retreat exit, optionally escalation-aware) and evaluate against the flip-only control on both event-study fixtures. The hinge is cutting the non-escalating branch cheaply. See [[event_study_anchor_findings]] bottom line.
...
**Continuous/version-control development:** older direct `emac` V1 runs in the universe-filter section are not the clean baseline. V4's older saved sequence (`a1688414` discarded config mismatch; `6108178a`, `eea81901`, `b12d3a50`, `bad11f56`, `f2299e5b`, `e43db176`, `43036e20`, `e58f3c5d`) primarily establishes transition correctness on sparse BTC fixtures.

**Frozen 1D version ladder (2026-08-10) — WINDOW-CONFOUNDED DIAGNOSTIC RUNS, NOT LIFESPAN/POPULATION EVIDENCE:** frozen 100-name `>$250k` Hyperliquid-volume snapshot; Binance USD-M; common end `2026-08-07T00:00:00Z`; EMA 10/200; `$10,000`; 100% equity at 1x; compounding; 5 bps fee/side + 5 bps slippage/side; no funding; summary retention; fixed floors `2500/2000/1500/1200/1000/800/600/500/300/201`. Each variant executed 96/96 planned cells across 11 batches. Explicit exclusions: SKR and MEGA below 201 bars; MNT and kNEIRO unsupported. Preserve these IDs for audit, implementation verification, and same-asset/same-window diagnostics only. Do not cite their breadth, winner membership, correlation, medians, or generalization as evidence. Standard metrics remain in the saved runs/UI.

- causal ETH full-retention acceptance: `8f7051b2-1987-4072-812d-0b849fcf894e`
- repaired `emac_cross`: `e56367fb-d8de-443f-99ce-116621b28460`, `b2840ef4-5274-4948-92e1-c9c2b5c3f6f5`, `8d09702d-1cbd-44c4-93e2-415d75ba8803`, `1bad79f1-1545-48a2-a569-0f5d0a1ff664`, `be0041c4-f64a-4bf4-99ab-9ef75e125453`, `e7e51536-1ab0-40e9-9276-93ae9acef3e7`, `bbe1f2ec-e2c5-47f9-b16c-84b901c63bd6`, `6bc6c218-6d17-4691-b8c2-448c976cb8c1`, `66ff2ce3-8077-41e4-a8ab-ee562fbc12ab`, `eb049363-7f14-4ec9-adfc-2690200e602b`, `2bfa45a4-274f-4604-a918-71a423484490`
- direct continuous `emac` V1: `5ab11145-e7a6-4c21-bedd-97132c42941c`, `33b4248f-676a-42c8-950e-c8c8adae3d82`, `b97666f7-fee1-467f-8647-d7327345fecd`, `418570a7-38e0-4689-b97b-6621c7f2c53a`, `bb31afeb-fb3b-4d5b-a72a-f687be9368c7`, `64395b79-3b09-4e54-a08a-37cf1b9001cc`, `22e5dcbe-f5f2-46c5-96fb-1cd77245829d`, `d1b5dd56-42d3-492b-845d-cc847b75e9e2`, `b392bcc1-a62d-42bc-bcc2-ea23f93e352a`, `33c6790e-e7b8-418f-b63a-b547c6494521`, `afc3d014-7079-42e0-a128-4113cef8b7b1`
- fixed-`±0.01` transition-only V4: `c629001b-9438-4070-88d4-7db0e016662f`, `4e7d05bf-8b88-4048-9f6c-80dc89636379`, `2113d85f-cc62-4a7f-8350-fafe98f7bcb8`, `c2ceb9ad-1622-4a09-8c72-a807d537b985`, `74dffb39-61cf-4f40-8155-1c7c3fcf8421`, `32e7ad01-de40-463d-8b8f-2cace0b5059e`, `dd7aed7a-dcf2-4292-9c56-0ab16b7ab955`, `dbd799e5-ac2a-4e87-bce9-fd3ec3bf1154`, `b850252f-064d-43b2-bcdd-ccc9122b7ea9`, `52c971c3-79c8-4b5b-8ea0-92df6020fa3c`, `a7e57026-447b-43be-9837-80c2634d4db1`
- fixed-`±0.01` continuous V5: `627260af-b2ac-4b34-92e0-0c5079368be1`, `50fc12ea-b11c-4e30-b482-8b723b20f58b`, `2900e56b-55a3-4bd2-8118-31d226b4041f`, `4ad31719-b716-4fee-8bbc-46a59c26386a`, `bdfc0698-91f1-45ca-927f-9b2d36927809`, `fd1e2e83-2716-4975-922d-d2f4e4c90515`, `0bfa93da-c6c5-404a-9563-c7d06c7e20b4`, `cbc5572d-2588-499d-9a39-dc583e83ba00`, `7ead3532-af4d-4c1e-b8f5-8e160b1efe19`, `7241a523-251e-4a22-b54e-d84fd03af0f5`, `e6f78bba-4161-4ce2-9bc2-34c01fcd4791`
...
### 2026-08-10 23:29 EDT

Completed and persisted the matched frozen-universe 1D version ladder after validating the causal `emac_cross` repair with full-retention ETH acceptance run `8f7051b2-1987-4072-812d-0b849fcf894e`. Repaired flip-only cross, direct continuous V1, fixed-`±0.01` transition-only V4, and fixed-`±0.01` continuous V5 each completed the same 96 planned asset-window cells across 11 lifespan-cohort batches without failure or liquidation. The canonical batch IDs and fixed assumptions are in the Run Registry above; the earlier broken-strategy batches remain inadmissible.

The provisional population-center read is that `±0.01` behaves as a near-zero threshold control, consistent with Destin's preregistered prediction that material effects should begin around `±0.05`–`±0.10`. V5 shows a small broad improvement over direct V1, while V4 stays near the cross control; continuous magnitude generally reduces drawdown but clips several large trend paths. All four variants remain negative-median over the informative cells, so none is promoted.

The similar positive-asset counts do not answer whether the same assets won. Two distinct questions are therefore parked: positive-set membership overlap and correlation of continuous asset-level outcomes, with lifespan cohort/window identity preserved. A common underlying trendability subset and mapping-specific substitution remain competing explanations. Destin explicitly paused computation until these thoughts and the runs were recorded. No overlap/correlation calculation and no `±0.05`/`±0.10` sensitivity run occurred.

### 2026-08-11 00:12 EDT

Destin identified a material fixed-floor regime confound and paused EMAC work to reconsider the batch planner. The fixed 2,500-bar 1D start is 2019-10-03, while the 2,000-bar start is 2021-02-14; assets with 2,001–2,499 trailing contiguous bars silently lose up to 499 available days, potentially excluding the March 2020 crash and early 2020–21 bull phase. The implementation does not use each asset's first/listing date as the run start: preflight counts contiguous candles backward from the fixed common end, assigns the largest floor cleared, and the packer calculates `start_ms = common_end_ms - floor × interval_ms`.

Decision: reclassify all four 1D run families above as **window-confounded diagnostic evidence**. Their exact same-asset/same-window pairing remains useful for implementation and conditional mapping comparisons, but their breadth, winner identity, correlations, population medians, and lifespan/generalization interpretations are quarantined. Do not run the parked correlation audit or `±0.05`/`±0.10` sensitivity until the planner's history-retention contract is accepted, a plan-only audit passes, and affected baseline cells are rerun.

Created [MON-214](https://linear.app/money-machine/issue/MON-214/redesign-binance-batch-planning-to-preserve-auditable-crypto-history) as the governing investigation/design issue. It documents the full bounded 5,000-bar availability check, the precise point where fixed-floor planning discards exact availability, the unresolved estimand and loss-policy choices, required per-asset plan audit fields, and acceptance criteria. MON-214 blocks MON-210, MON-211, and MON-165 and is related to the shipped MON-143/MON-173/MON-176 infrastructure.
